...
import { tick,nvtsLogo,taLogo } from './baseSixtyFourImage'
@Injectable({
  providedIn: 'root',
})
export class CreatePDFService {
  ...
  generatePDF(
    ...
    return new Promise((resolve, reject) => {
      ...
      const img = new Image();
      img.src =`data:image/png;base64,${taLogo}`;;

      const checkImg = new Image();
      checkImg.src = `data:image/png;base64,${tick}`;
      const nmvtlogo = `data:image/png;base64,${nvtsLogo}`;
